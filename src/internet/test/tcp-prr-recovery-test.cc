...
public:
   ...
   * \para lastDeliveredBytes Bytes delivered on last acknowledgment.
...
private:
  ...
  uint32_t m_lastDeliveredBytes;      //!< Bytes delivered on last acknowledgment.
...
PrrRecoveryTest::PrrRecoveryTest (uint32_t cWnd,
  ...
  : TestCase (name),
    ...
    m_lastDeliveredBytes (lastDeliveredBytes),
...
PrrRecoveryTest::DoRun ()
{
  ...
  recovery->EnterRecovery (m_state, m_unAckDataCount, false, 3, m_bytesInFlight, m_lastDeliveredBytes);
  ...
  recovery->DoRecovery (m_state, m_unAckDataCount, false, 3, m_bytesInFlight, m_lastDeliveredBytes);
